...
public class HqlQueryFormatterTest {

	private HqlQueryFormatter formatter;

	@BeforeEach
	void setup() {
		formatter = new HqlQueryFormatter();
	}
	...
	@Test
	void deleteStatementWithoutFrom() {
		assertEquals("""
				DELETE Employee e
				WHERE e.active = false""",
				formatter.format("DELETE Employee e WHERE e.active = false"));
	}
	...
	void invalidQueryReturnsOriginal() {
		String invalid = "NOT A VALID HQL QUERY !!!";
	...
	void queryWithCrossJoin() {
		assertEquals("""
				...
				WHERE e.active = true""",
				formatter.format("SELECT e, d FROM Employee e CROSS JOIN Department d WHERE e.active = true"));
	}

	@Test
	void queryWithLimitOffset() {
		assertEquals("""
				SELECT e
				FROM Employee e
				ORDER BY e.name
				LIMIT 10
				OFFSET 20""",
				formatter.format("SELECT e FROM Employee e ORDER BY e.name LIMIT 10 OFFSET 20"));
	}
	...
	void fromQueryStyle() {
		assertEquals("""
				FROM Employee e
				WHERE e.active = true""",
				formatter.format("FROM Employee e WHERE e.active = true"));
	}
