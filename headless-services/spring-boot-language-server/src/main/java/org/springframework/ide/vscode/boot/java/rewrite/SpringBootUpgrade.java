...
public class SpringBootUpgrade {
	...
	private Recipe createUpgradeRecipe(Version version, Version targetVersion) {
		...
		if (version.getMajor() == targetVersion.getMajor() && version.getMinor() == targetVersion.getMinor()) {
			...
			recipe.getRecipeList().add(new org.openrewrite.maven.UpgradeDependencyVersion("org.springframework.boot", "*", targetVersion.toString(), null, null, null));
			recipe.getRecipeList().add(new UpgradeParentVersion("org.springframework.boot", "spring-boot-starter-parent", targetVersion.toString(), null, null));
		}
